\part*{Lecture 1: Spot Discount and Forward Rates}
\addcontentsline{toc}{part}{Lecture 1: Spot Discount and Forward Rates}
\beginlecture{01}
...
through the published course site at \texttt{markhendricks.github.io/finm-fixedincome},
he opened the first content notebook --- \emph{1.1.~The Spot Discount Curve} ---
...
\section{The Spot Discount Curve}\label{L1-sec:spot}

\subsection{The Spot Curve and No-Arbitrage}\label{L1-sec:spot-noarb}

\begin{keyblock}
\textbf{Cell~1.} \emph{(Page~1.1 \textperiodcentered{} setup)}\\
\noindent The notebook opens with the standard scientific-Python setup,
loads the day's Treasury quote file, and renames the simplified columns
(\texttt{quote date}, \texttt{cpn rate}, \texttt{dirty price}, \dots) onto
the CRSP-style names (\texttt{CALDT}, \texttt{TCOUPRT}, \texttt{TDNOMPRC},
\dots) that the existing \texttt{cmds.treasury\_cmds} helpers expect.
The first non-trivial output is the head of the loaded quote frame:
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c02_load_quotes.png}\end{center}
The screenshot crops the right-hand columns (\texttt{ytm}, \texttt{accrued
int}, \texttt{total size}); for the full table see the live page at
\texttt{discussions/1.1.~The Spot Discount Curve.html}, top-of-page
preamble, the cell that calls \texttt{metrics.head()}.
\end{keyblock}
...
--- with quote prices already split into \emph{clean} and \emph{dirty}
...
\begin{keyblock}
\textbf{Cell~4--9.} \emph{The Spot Curve and No-Arbitrage.}\\
\textbf{Above we saw that:}
\begin{itemize}
  \item YTM is an alternate way to quote a price.
  \item For a zero-coupon bond held to maturity, it is the annualized return.
  \item Beyond that, YTM is not useful.
  \item In particular, YTM is not useful for pricing other securities.
\end{itemize}
...
\textbf{No arbitrage.} No-arbitrage conditions say that two risk-free cashflows
...
is a single number that, when used to discount \emph{all} of a bond's cashflows,
reproduces its market price. That makes YTM a perfectly valid summary of the
bond, but a terrible discount rate for any \emph{other} cashflow. If you took
...
The way out is to demand a discount rate that depends only on \emph{when} the
...
\begin{tipblock}
\textbf{Filling the gap.} The lecturer said ``YTM assigns a discount rate to
...
\]
The same $y_j$ discounts \emph{every} cashflow. If two bonds with different
coupon schedules trade at the same time, they will generally have different
YTMs, even though no-arbitrage demands a \emph{single} rate per maturity. That
contradiction is precisely the symptom that YTM is not the right object for
cross-bond pricing.
\end{tipblock}

\subsection{The Spot Curve}\label{L1-sec:spot-curve}

\begin{keyblock}
\textbf{Cells~11--19.} \emph{The Spot Curve definition.}\\
...
rate $r(t,T_i)$ \emph{does not} depend on the specific bond $j$; it is a
function of the cashflow timing $T_i$ alone.

\textbf{Reminder: YTM Formula.} Define the yield-to-maturity for bond $j$ as
...
\emph{one} unknown per bond ($y_j$) and is solved bond-by-bond. The spot-curve
equation has \emph{many} unknowns ($r(t,T_i)$ for every cashflow date appearing
...
\subsection{The Discount Curve}\label{L1-sec:disc-curve}

\begin{keyblock}
\textbf{Cells~20--24.} \emph{The Discount Curve and compounding.}\\
\textbf{Discount Factor vs Compounded Rates.} All compounded rates imply the
...
point: the \emph{discount factor} $Z(t,T)$ is the primitive object. It does
...
He then made an aside that recurs throughout the course: \emph{multiplicative
...
\begin{keyblock}
\textbf{Cells~25--27.} \emph{Example.}\\
Consider the annually compounded spot rate of 5\% at maturity $T-t=1$.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_compounding.png}\end{center}
...
The lecturer pointed at the \texttt{discount} column and asked the class
whether they were surprised to see all 95.2381\%. Some were. The exercise
exists, he said, to drive home that the rate is a \emph{label} for a discount,
...
\subsection{The Curve}\label{L1-sec:curve}

\begin{keyblock}
\textbf{Cells~28--32.} \emph{The Curve.}\\
...
\subsection{Coupon Bonds and the Modeling Pipeline}\label{L1-sec:coupon}

\begin{keyblock}
\textbf{Cells~33--34.} \emph{Coupon bond.}\\
\textbf{Use.} If we estimate the discount curve (equivalent to estimating the
...
\subsection{Modeling the Spot Curve --- Filtering and Cashflow Mapping}\label{L1-sec:filter-map}

\begin{keyblock}
\textbf{Cells~35--36.} \emph{Modeling the Spot Curve.}\\
\textbf{Filter Data.} Filter to eliminate maturities that are too short or
too long, quotes that do not have transparent pricing, and so on.
\end{keyblock}

\begin{keyblock}
\textbf{Cells~37--38.} \emph{Map Cashflows.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c38_cashflows.png}\end{center}
The output of \texttt{calc\_cashflows} is the cashflow matrix $\mathbf{C}$:
...
\texttt{discussions/1.1.~The Spot Discount Curve.html}, section
\emph{Map Cashflows}.
\end{keyblock}
...
\begin{itemize}
  \item \textbf{Yield filter.} Drop bonds with implausible quoted yields
        (negative, or wildly off the visible cluster). These are usually
        stale quotes or special-issue bonds.
  \item \textbf{TIPS filter.} Drop TIPS. They are inflation-linked and would
        be an apples-to-oranges comparison with the nominal curve we are
        building.
  \item \textbf{Maturity filter.} Drop the very short end (where a single
        bill near maturity has wild quote noise) and bonds beyond a chosen
        horizon.
  \item \textbf{Date-uniqueness restrictions.} Two restrictions become important
        for the bootstrap below: \texttt{RESTRICT\_DTS\_MATURING} requires at
        least one (and at most one) bond maturing on each date in scope, and
        \texttt{RESTRICT\_REDUNDANT} drops bonds whose cashflow rows are
        linearly dependent on others already in the system.
\end{itemize}

\subsection{Estimating the Discounts}\label{L1-sec:estimate}

\begin{keyblock}
\textbf{Cells~39--42.} \emph{Estimating the discounts.}\\
Notation:
\begin{itemize}
  \item $\boldsymbol{p}$: $n\times 1$ vector of the price for each issue.
  \item $\boldsymbol{z}$: $k\times 1$ vector of the discount factors for each
        relevant date (one per cashflow column of $\mathbf{C}$).
  \item $\boldsymbol{C}$: $n\times k$ cashflow matrix.
\end{itemize}
...
underlined the question that drives the next several subsections: \emph{what
...
\emph{not} an econometric model in the regression-with-noise sense; it is a
...
\begin{keyblock}
\textbf{Cells~43--45.} \emph{Which estimation?}\\
What if $\mathbf{C}$ is square, $n=k$; tall, $n>k$; wide, $k>n$?

\textbf{Identified.} Suppose $\mathbf{C}$ is linearly independent. Then we can
solve uniquely for $\boldsymbol{z}$.

\textbf{Unidentified.} The discount curve is not identified --- think
...
identification fixes; the right move is to impose \emph{structure on the
...
\subsection{Perfect Estimation: Bootstrap and OLS}\label{L1-sec:perfect}

\begin{keyblock}
\textbf{Cells~46--50.} \emph{Perfect Estimation of Spot Curve.}\\
\textbf{Bootstrap.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c48_bootstrap.png}\end{center}
\textbf{OLS Estimation.} Same restrictions, model swapped for OLS.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c50_ols.png}\end{center}
\end{keyblock}
...
\begin{tipblock}
\textbf{Filling the gap.} The bootstrap is the iterative procedure that solves
the system one date at a time, starting from the shortest maturity:
\begin{enumerate}
  \item Take the bond with the earliest maturity $T_1$. Its only cashflow is
        the final principal-plus-coupon, so the price gives $Z(t,T_1)$
        directly: $P_1 = (\text{cf})\, Z(t,T_1)$.
  \item Take the bond with the next earliest maturity $T_2$. Its cashflows are
  ...
  \item Continue forward in maturity, at each step using all already-known
        $Z(t,T_i)$ to back out the new $Z(t,T_j)$.
\end{enumerate}
The procedure requires \emph{exactly one} bond maturing at each $T_j$ in scope
--- which is what the \texttt{RESTRICT\_DTS\_MATURING} flag enforces --- and
...
\subsection{Spot Curve Output}\label{L1-sec:spot-output}

\begin{keyblock}
\textbf{Cells~51--55.} \emph{Spot Curve.}\\
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c54_spot_curve.png}\end{center}
The estimated spot curves under bootstrap and OLS are nearly identical when
the date-uniqueness restrictions are imposed.
\end{keyblock}

\begin{checkpoint}
\textbf{My note.} The lecturer's quick observation that ``the bootstrap and
OLS curves are nearly identical'' is logically necessary, not coincidence:
when $\mathbf{C}$ is square and full-rank, the OLS solution
$(\mathbf{C}^\top\mathbf{C})^{-1}\mathbf{C}^\top\boldsymbol{p}$ collapses to
$\mathbf{C}^{-1}\boldsymbol{p}$, which is exactly what the bootstrap iterations
compute. They differ only because the OLS pipeline allows the final filter to
admit a slightly different bond set, so the matrices are not bit-identical.
\end{checkpoint}

\subsection{Why not Bootstrap the Entire Curve?}\label{L1-sec:why-not}

\begin{keyblock}
\textbf{Cells~56--71.} \emph{Why not bootstrap the entire curve?}\\
...
\texttt{RESTRICT\_DTS\_MATURING = False} and
\texttt{RESTRICT\_REDUNDANT = False} (see the live page at
\texttt{discussions/1.1.~The Spot Discount Curve.html}, section
\emph{Why not bootstrap the entire curve?}, the two cells immediately
under that heading). The first plot below is the resulting OLS discount
curve under the relaxed restrictions:
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c62_estimate_bad.png}\end{center}

The OLS-extracted discount curve under the relaxed restrictions
\emph{vacillates extremely} in the first 5 years and is seemingly missing
discounts between 10 and 15 years.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_vacillating.png}\end{center}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c71_vacillate_long.png}\end{center}
...
\emph{ordinary least squares} gives you when you feed it the full quote
file.'' His phrasing: \emph{the non-smoothness here is absurd}, and what
...
\subsection{Multicollinearity}\label{L1-sec:multicol}

\begin{keyblock}
\textbf{Cells~72--78.} \emph{Multicollinearity.}\\
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c78_condnum.png}\end{center}
...
\begin{tipblock}
\textbf{Filling the gap.} The lecturer mentioned ``$x'x$'' in passing but did
...
\subsection{Nelson--Siegel Model of the Yield Curve}\label{L1-sec:NS}

\begin{keyblock}
\textbf{Cells~79--92.} \emph{Nelson--Siegel.}\\
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c92_ns_params.png}\end{center}
...
parameter table is on the live page at \texttt{discussions/1.1.~The Spot
Discount Curve.html}, section \emph{Nelson Siegel Model of the Yield Curve}.
\end{keyblock}

The lecturer made a point of explaining each parameter's role:

\begin{itemize}
  \item $\theta_0$ is the long-run level. As $\tau\to\infty$, the second and
        third terms collapse and $r(t,T)\to\theta_0$.
  \item $\theta_1$ is the short-end premium. As $\tau\to 0$,
        $r(t,T)\to\theta_0 + \theta_1$, so the model's instantaneous spot is
        $\theta_0+\theta_1$.
  \item $\theta_2$ controls the hump in the middle of the curve. The function
  ...
  \item $\theta_3$ is a horizon scaling that determines where the hump sits.
...
\subsection{Fitted Spot and Discount Curves}\label{L1-sec:fitted}

\begin{keyblock}
\textbf{Cells~93--98.} \emph{Use these fitted parameters to calculate the
Spot Curve, then convert to the Discount Curve.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c96_ns_spot.png}\end{center}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_c98_ns_disc.png}\end{center}
The parameterized functional form lets us evaluate the curve at \emph{any}
...
\subsection{Spot Curve vs YTM Curve}\label{L1-sec:vs-ytm}

\begin{keyblock}
\textbf{Cells~99--105.} \emph{Spot Curves vs YTM.}\\
We immediately calculate the YTM for each issue, and plotting these YTMs
against maturity gives a curve. \emph{This YTM curve is not the same as the
spot curve} (often called the \emph{yield curve}, confusingly).
...
\begin{tipblock}
\textbf{Filling the gap.} The lecturer said ``YTM is a certain average of
semi-compounded spot rates'' and moved on. The precise statement: for a
zero-coupon bond at maturity $T$, the YTM \emph{equals} the spot rate $r(t,T)$
...
\subsection{Model Prices and Errors}\label{L1-sec:errors}

\begin{keyblock}
\textbf{Cells~106--114.} \emph{Model Prices and Errors.}\\
How well does Nelson--Siegel fit market prices? How well do the bootstrap
and OLS do? The notebook defines a helper \texttt{compare\_pricing} (a
function-definition cell with no output of its own) and then calls
\texttt{plot\_prices(price\_comp\_*, label)} four times, once per fitted
...
at \texttt{discussions/1.1.~The Spot Discount Curve.html}, section
\emph{Model Prices and Errors}.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_1_errors.png}\end{center}
...
sees \emph{exactly} when the restrictions are tight enough, and OLS comes
...
material with: \emph{a parametric model trades in-sample fit for usability
out of sample}. Every fixed-income model in the rest of the course will
involve some version of this tradeoff.

\section{Forward Rates}\label{L1-sec:fwd}

\subsection{Motivation}\label{L1-sec:fwd-motiv}

\begin{keyblock}
\textbf{Cells~0--4.} \emph{Forward Rates.}\\
A given date's term structure encodes information about rates in the future.

\textbf{Example.} Suppose an investor will be receiving (risk-free) \$100
...
arriving in six months and wants to commit \emph{today} to where it will sit
...
\subsection{Looking at the Data}\label{L1-sec:fwd-data}

\begin{keyblock}
\textbf{Cells~5--7.} \emph{Looking at the Data.}\\
Consider the quoted treasuries on the date shown below.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_quotes.png}\end{center}
\end{keyblock}
...
\subsection{The Answer: Forward Rate via Replication}\label{L1-sec:fwd-answer}

\begin{keyblock}
\textbf{Cells~8--13.} \emph{The answer.}\\
The rate we can guarantee today, effective starting 6 months from now,
through 12 months from now, is given below.
\begin{itemize}
  \item Sell a forward contract today ($t$) to receive 100m in 6 months for
        repayment in 12 months with the TBD forward interest rate.
  \item Simultaneously at $t$, sell short the 6mo T-bill and use the
        proceeds to invest in the 12mo T-bill.
  \item At 6mo, use the proceeds from the forward counterparty to repay the
        6mo treasury.
  \item At 12mo, use the proceeds from the 12mo T-bill to pay off the
        forward loan.
\end{itemize}
We have a perfectly hedged position at all times. Thus, the forward rate must
equal the gain from the 12mo tbill.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c13_forward_calc.png}\end{center}
\end{keyblock}
...
\emph{is} a 6mo--12mo loan at the rate that solves
$P_2/P_1 = 1/(1+f/2)$, hence the forward rate.

\begin{tipblock}
\textbf{Filling the gap.} The lecturer wrote ``the forward rate must equal
...
\subsection{The Guaranteed Forward Rate}\label{L1-sec:fwd-guar}

\begin{keyblock}
\textbf{Cells~14--16.} \emph{The guaranteed forward rate.}\\
Given this forward dollar exposure, the investor is guaranteed the following
semiannual compounded rate:
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c16_rate_summary.png}\end{center}
\end{keyblock}

\subsection{Interim Risk}\label{L1-sec:fwd-interim}

\begin{keyblock}
\textbf{Cells~17--21.} \emph{Interim Risk.}\\
Above, we did not deal with the price of the forward replication \emph{but
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c21_tsdata.png}\end{center}
\end{keyblock}

The lecturer was careful to distinguish two things that students often
conflate. \emph{The forward rate is locked in today.} The eventual cashflow
stream at $T_1$ and $T_2$ is exactly the $-100$m / $+100\text{m}\cdot p_1/p_2$
pattern, no matter what happens in between. \emph{But the mark-to-market
...
\subsection{Calculating Forward Rates}\label{L1-sec:fwd-calc}

\begin{keyblock}
\textbf{Cells~22--27.} \emph{Calculating Forward Rates.}\\
Extract the forward rate from the spot curve $r(t,T)$, or equivalently, from
the discount curve $Z(t,T)$.

\textbf{Forward Discount Factors.} Define
\[
    F(t,T_1,T_2) \;=\; \frac{Z(t,T_2)}{Z(t,T_1)}.
\]
\textbf{Forward Discount Rates.} The forward discount rate is given as
\[
...
\subsection{The Forward Curve}\label{L1-sec:fwd-curve}

\begin{keyblock}
\textbf{Cells~28--32.} \emph{The Forward Curve.}\\
...
\emph{derivative} of the spot rate (in the calculus sense, scaled by $\Delta$),
...
\subsection{Extracting the Forward Curve from the Spot Curve}\label{L1-sec:fwd-extract}

\begin{keyblock}
\textbf{Cells~33--42.} \emph{Extracting the forward curve from the spot curve.}\\
...
\texttt{quote\_dates = [`2006-12-29', `2018-12-31', `2020-12-31',
`2022-12-30', `2023-11-30']} and examining \texttt{2023-11-30}; the
exact \texttt{extract\_spot\_curves} call is on the live page at
\texttt{discussions/1.2.~Forward Rates.html}, section \emph{Extracting
the forward curve from the spot curve}.
\end{keyblock}

\begin{keyblock}
\textbf{Cells~39--41 (output).}\\
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_spot.png}\end{center}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_forward.png}\end{center}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c41_fwd_disc.png}\end{center}
...
top of the spot curve. The forward curve sits \emph{above} the spot curve
where the spot curve is rising (early- and mid-curve) and crosses below where
it flattens out at the long end.

\textbf{Relationship between the spot and forward curves.} The forward curve
...
\subsection{Compounding for Forward Rates}\label{L1-sec:fwd-compound}

\begin{keyblock}
\textbf{Cells~43--50.} \emph{Compounding.}\\
...
\emph{always store $F$ internally and convert to a rate at the boundary of
your computation when you need to display or compare}.

\subsection{Extracting the Spot Curve from the Forward Curve}\label{L1-sec:fwd-spot}

\begin{keyblock}
\textbf{Cells~51--57.} \emph{Extracting the spot curve from the forward curve.}\\
It is possible to ``bootstrap'' the spot curve from forward rates. Consider:
\[
    Z(0,T_n) \;=\; Z(0,T_{n-1})\, e^{-f(0,T_{n-1},T_n)\,(T_n - T_{n-1})}.
\]
\textbf{Converting to the spot rate:}
\[
...
\subsection{A Smooth Spot Curve?}\label{L1-sec:fwd-smooth}

\begin{keyblock}
\textbf{Cells~58--61.} \emph{A Smooth Spot Curve?}\\
...
forward curve.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c61_extract_loop.png}\end{center}
\end{keyblock}
...
\subsection{Forward Rate Agreements}\label{L1-sec:FRA}

\begin{keyblock}
\textbf{Cells~62--64.} \emph{Forward Rate Agreements.}\\
\textbf{Definition.} A Forward Rate Agreement (FRA) is similar to the
...
\textbf{No-arbitrage replication.} Like the forward loan described at the
...
\subsection{Forward Contracts on Treasury Bonds}\label{L1-sec:fwd-contract}

\begin{keyblock}
\textbf{Cells~65--72.} \emph{Forward Contracts.}\\
\textbf{Definition.} A forward contract is an agreement to buy a bond with
...
\textbf{Initialization.} The forward price is set such that there is no
upfront price to initiate the forward.

\textbf{Final payoff.} The final payoff from the long forward contract is
$P(T_0, T) - P_{\text{Fwd}}(t,T_0,T)$.

\textbf{Price vs Value.} As with FRAs, it is important to distinguish the
forward \emph{price} (the agreed exchange price) from the forward
\emph{value} (the mark-to-market of the contract).
\end{keyblock}

\begin{tipblock}
\textbf{Filling the gap.} The lecturer used ``price'' and ``value''
distinctly without writing the formal distinction. The forward \emph{price}
...
contract has zero value at initiation. The forward \emph{value}
...
\subsection{Pricing Forward Contracts on Treasuries}\label{L1-sec:fwd-treas}

\begin{keyblock}
\textbf{Cells~73--82.} \emph{Forward Contracts on Treasury Bonds --- Pricing.}\\
Pricing forward contracts on Treasuries is straightforward: \emph{just use
the forward discount} $F(t,T_0,T)$ \emph{instead of} $Z(t,T)$.

\textbf{Zero coupon bond.} Consider a zero-coupon bond maturing at $T$ with
face value 100. The forward price is
\[
    P_{\text{Fwd}}(t,T_0,T) \;=\; 100 \, F(t,T_0,T).
\]
\textbf{Coupon bond.} Consider coupons at $T_i$ with final principal at $T$:
\[
    P_{\text{Fwd}}(t,T_0,T)
...
\textbf{Example.} The forward contract can achieve the same as the FRA
...
$T_2$-T-bill at $T_1$.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c83_display_bonds.png}\end{center}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c86_fwd_contract.png}\end{center}
\end{keyblock}
...
\subsection{Appendix: Mathematical Details on FRAs}\label{L1-sec:fra-math}

\begin{keyblock}
\textbf{Cells~87--93.} \emph{Appendix: FRA Value.}\\
...
\emph{four} dates: the initialization date $t=0$, the valuation date $t$, the
spot rate fix date $T_1$, and the cashflow date $T_2$.
\end{keyblock}

\begin{keyblock}
\textbf{Cells~96--100.} \emph{Boundaries.}\\
...
rate spread.
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L1_2_c100_fra_dates.png}\end{center}
\end{keyblock}

The lecturer's summary: the FRA's value is a forward-rate \emph{spread}
...
\subsection{Appendix: No-Arbitrage Construction of the Forward Rate}\label{L1-sec:fwd-noarb}

\begin{keyblock}
\textbf{Cells~110--124.} \emph{Forward Rates via No-Arbitrage.}\\
...
two coloured tables (\texttt{arb\_chart['price']} highlighted in light yellow,
\texttt{bank\_position} highlighted in light blue) and walked the class
...
the forward rate is \emph{not} the market's expectation of the future spot
