\part*{Lecture 3: Treasury Arbitrage and STIR Futures}
\addcontentsline{toc}{part}{Lecture 3: Treasury Arbitrage and STIR Futures}
\beginlecture{03}
...
\emph{Treasury Arbitrage in~2008} HBS case (Parts~A and~B) and the live
spreadsheet the lecturer shared. The remainder of the lecture covers
notebooks~3.1~\emph{STIR Futures}, 3.2~\emph{Inferring FF Futures}, and the
calibration half of 3.3~\emph{Discount via SOFR Futures}; the deep dive into
the treasury-futures basket and the cheapest-to-deliver trade starts on
notebook~4.2 and is deferred to Lecture~4.

\section{Case: Treasury Arbitrage in 2008}\label{L3-sec:case}

\subsection{Two Treasuries, Same Maturity Date}\label{L3-sec:case-setup}
...
November~2008 is: \emph{given that these cashflows must converge on the same
...
immediately. Dirty prices are \emph{not} comparable across bonds with
...
is the YTM, because YTM is a \emph{re-scaled} price that factors out the
cashflow schedule. Once the YTMs are on the table, the trade idea is:
\emph{buy low, sell high, but in YTM space the direction flips.} A high YTM
...
\begin{tipblock}
\textbf{Filling the gap.} The lecturer's slogan ``YTM is a rescaled price''
...
bundles have the same \emph{average} discount rate even though they differ
...
\subsection{Sizing the Trade}\label{L3-sec:case-size}

With the direction of the trade settled, the lecturer walked through how to
\emph{size} it. Three candidate rules were discussed, each one better than
the last:

\begin{enumerate}
  \item \emph{One contract long, one short.} This is the dumbest thing you
        could do. The bonds trade at \$106 and \$144 per \$100 face, so this
        is not even dollar-neutral.
  \item \emph{Dollar-neutral.} Buy \$1 of the bond, short \$1 of the note.
  ...
  \item \emph{Dollar-duration-neutral} (``galaxy-brain,'' in the lecturer's
        phrase). The hedge ratio is chosen so that the \emph{dollar}
        dollar-duration of the long leg equals the dollar dollar-duration of
        the short leg.
\end{enumerate}

A subtlety the lecturer insisted on: duration or \emph{modified} duration?
...
securities with the \emph{same maturity}, not on the evolution of the spot
...
\begin{keyblock}
\textbf{The sized trade (\$1M own capital, 2\% repo haircut).}
\[
...
\subsection{The Expected Profit}\label{L3-sec:case-pnl}
...
\begin{tipblock}
\textbf{Filling the gap.} The 25K-per-basis-point sensitivity is the
...
\subsection{What Actually Happened}\label{L3-sec:case-what-happened}

The YTM spread did not converge. It \emph{widened}. From the 35~bp level in
...
\begin{checkpoint}
\textbf{My note.} The arithmetic is symmetric: the same modified-duration
machinery that promises \$871K on convergence threatens identical-magnitude
losses on divergence. The lecturer emphasized that a trader tempted to
\emph{double down} at 70~bp, on the grounds that the opportunity has grown,
is making a statistical argument that was already wrong at 35~bp.
\end{checkpoint}
...
a margin call for the difference because this is \emph{overnight} repo,
...
firm pulls the plug, the trader has to \emph{unwind} at the worst possible
...
my competitors to beat me in. But I did need \emph{someone} to come along
...
\subsection{Was This Really Arbitrage?}\label{L3-sec:case-arb}
...
profit --- about 2.5\% in total. That \emph{is} a mathematical arbitrage. It
...
\emph{leverage} --- 50-to-1 via the 2\%~haircut on overnight repo. And the
...
\begin{checkpoint}
\textbf{My note.} The homework exercise re-runs the same setup in 2023 with
a 2019 10-year note and a 1993 30-year bond as the pair --- the same
structural asymmetry (on-the-run note vs way-off-the-run bond). The
lecturer said they picked the fattest spread they could find in recent
years; even so, the 2008-sized dislocation was not on offer. The exercise
is there to drive home the sizing machinery and to show that, absent a
crisis, the spread stays in a tight band.
\end{checkpoint}

The lecturer closed the Tuesday session with the participation code
\texttt{treasury-basis}, the logistics of moving from Group~A to Group~B
...
\section{STIR Futures}\label{L3-sec:stir}

Wednesday morning opened with the class looking at the course webpage at
\texttt{discussions/3.1.~STIR Futures.html}. The lecturer referred several
...
\subsection{STIR Futures --- Overview and Payoff Convention}\label{L3-sec:stir-overview}

\begin{keyblock}
\textbf{Cells~1--9.} \emph{(Page 3.1 \textperiodcentered{} STIR Futures.)}\\
\noindent \textbf{Short-Term Interest Rate (STIR) Futures} are contracts
...
Similarities:
\begin{itemize}
  \item Trade on an exchange.
  \item Daily settlement.
\end{itemize}
Differences:
\begin{itemize}
  \item Cash settled, so ``delivery'' is simple cash payment.
  \item No embedded options.
\end{itemize}

Specs across most STIR contracts:
\begin{itemize}
  \item Delivery is last day of the contract month.
  \item Payoffs are \emph{inversely} related to the reference rate:
    \[
      N\bigl(1 - r^{\text{ref}}_T\bigr).
    \]
\end{itemize}
Thus, when \textbf{rates go up, the futures value goes down}.
\end{keyblock}
...
is \emph{realized}, not paper --- the exchange moves cash from the loser's
...
\emph{every} fixed-income product behave the same way under rate moves.
...
\begin{tipblock}
\textbf{Filling the gap.} The affine form also makes the STIR future
\emph{hedgeable} against a floating-rate loan. A lender who has extended
...
\subsection{Fed Funds Futures}\label{L3-sec:stir-fff}

\begin{keyblock}
\textbf{Cells~10--18.} \emph{Fed Funds Futures (FFF).}\\
\textbf{Reference rate:}
\begin{itemize}
  \item Simple average of the Fed Funds effective rate.
  \item Every day in the month is counted in the simple average.
  \item Non-trading days take the value of the most recent traded day (so
        that value is counted multiple times).
\end{itemize}
\textbf{Sizing:}
\begin{itemize}
  \item Notional $N = \$5{,}000{,}000$.
  \item Settlement horizon one month, 30/360 day-count.
\end{itemize}
...
\end{align*}
Each \textbf{increase} of 1~bp in the reference index is
\[
  \mathrm{PnL} \;=\; -\$41.67.
\]
\end{keyblock}
...
\emph{funds effective} rate --- the rate at which bank~A lends to bank~B
...
\begin{tipblock}
\textbf{Filling the gap.} The factor of $\tfrac{30}{360}$ is not really
...
\subsection{Market Expectations from FFF}\label{L3-sec:stir-market-exp}

\begin{keyblock}
\textbf{Cell~19.} \emph{Market Expectations.}\\
...
\subsection{SOFR --- Rate and Futures}\label{L3-sec:stir-sofr}

\begin{keyblock}
\textbf{Cells~20--23.} \emph{Understanding SOFR.}\\
\textbf{What is SOFR?} The \textbf{Secured Overnight Financing Rate}
(SOFR) is a broad measure of the cost of borrowing cash overnight,
collateralized by U.S.~Treasury securities. It is:
\begin{itemize}
  \item Published daily by the Federal Reserve Bank of New York.
  \item Based on actual transactions in the Treasury repo market
        ($\sim$\$1 trillion/day).
  \item Nearly risk-free (secured by Treasuries).
\end{itemize}
...
\textbf{SOFR vs.\ Fed Funds.}
\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Aspect} & \textbf{Fed Funds} & \textbf{SOFR} \\
\midrule
...
\textbf{Compounded SOFR.} For 3-month SOFR futures, the reference rate is
the \emph{compounded return} of daily SOFR over the quarter. Daily
...
\begin{keyblock}
\textbf{Cells~24--28.} \emph{SOFR Futures.}\\
The CME trades both 1-month and 3-month SOFR futures.

\textbf{1-month SOFR}. Contract is nearly identical to the Fed Funds
Futures contract above. SOFR was designed to mimic the notional and
mechanics of FFF.

\textbf{3-month SOFR}. Reference rate is the compounded return of SOFR
...
\]
\textbf{Timing:} offered and settled on a quarterly schedule
(mid-Mar/Jun/Sep/Dec). \textbf{Sizing:} $N = \$1{,}000{,}000$, horizon
...
\emph{First}, it trades far more than the 1-month. The old liquidity
...
\emph{Second}, the reference rate is the \emph{compounded} return of
...
\emph{Third}, the notional is \$1M rather than \$5M, and the horizon is
...
\begin{tipblock}
\textbf{Filling the gap.} Why compounding on 3-month SOFR but not on
...
additional subtlety. Once the compounding period has \emph{started}, there
...
\subsection{Technical Points --- Discount Curves, Convexity, Basis Risk}\label{L3-sec:stir-tech}

\begin{keyblock}
\textbf{Cells~29--32.} \emph{Technical Points.}\\
\textbf{STIR futures for estimating discounts.} For any pricing, we need
...
\textbf{Convexity adjustment.} Futures rates are higher than forward
rates:
\begin{itemize}
  \item Daily settlement is adverse for the long position, as seen with
        bond futures.
  \item This price difference reverses for the rate difference, given the
        specification of the contract.
\end{itemize}
It is common to use the following \textbf{convexity adjustment}
\[
...
\textbf{Basis risk.} Futures hedging strategies leave basis risk:
\begin{itemize}
  \item The forward contract is OTC and customizable, so it can be
        written on the exact interest-rate risk.
  \item Using futures, one must use whichever contract is closest (a
        similar bond, a similar interest rate).
  \item This may leave basis between the futures underlying and the
...
fixed-income desks the discount curve is built \emph{piecewise}: STIR
...
quoted price rises), and this increment arrives in cash \emph{today};
...
a \emph{higher} implied rate. Hence $r^{\text{fut}} > r^{\text{fwd}}$, and
a desk that reads rates off the futures chain and treats them as forwards
will systematically \emph{overstate} the short-end of the forward curve.

\begin{tipblock}
\textbf{Filling the gap.} The quadratic $\tfrac12 \tau^2 \sigma^2$ form is
...
\section{Inferring the Fed Funds Path}\label{L3-sec:infer}

\begin{keyblock}
\textbf{Cells~1--2.} \emph{(Page 3.2 \textperiodcentered{} Inferring FF
Futures.)}\\
The data has market data on the following:
\begin{itemize}
  \item Fed Fund Futures chain out 18 months.
  \item Dates of upcoming Fed meetings (approximated in 2026).
  \item Spot Fed Funds data.
  \item Prices of the futures chain on a historic date.
\end{itemize}
\end{keyblock}
...
\subsection{The Fed Funds Curve as a Menu}\label{L3-sec:infer-menu}

\begin{keyblock}
\textbf{Cell~3.} \emph{Chart the Fed Futures Rates.}\\
We will chart the Fed Funds curve at \texttt{2025-01-26} and
\texttt{2025-01-31}.
Note that we are charting the implied Fed Funds Futures \emph{rate}, not
price.
\end{keyblock}

The lecturer was emphatic on an easily confused distinction. A plot of
futures prices across contract months is \emph{not} a time series --- it
is a \emph{menu}. At a single instant, the market offers a spectrum of
...
\emph{forward curve} for the underlying rate. Reading the January 2025
...
\subsection{Extracting the Expected Path}\label{L3-sec:infer-path}

\begin{keyblock}
\textbf{Cells~4--7.} \emph{Extracting the Expected Path of Fed Funds Rates.}\\
Assume the Fed:
\begin{itemize}
  \item sets the rate exactly at its list of meeting dates;
  \item does not change the rate between meeting dates.
\end{itemize}
...
\emph{with} a meeting, the reference rate is a weighted average of the
...
\begin{keyblock}
\textbf{Cells~8--9.} \emph{Technical Point.}\\
...
\begin{enumerate}
  \item Use the futures rate from the $(t+1)$-contract.
  \item Calculate the implied rate for the remainder of month $t$,
...
\begin{tipblock}
\textbf{Filling the gap.} The ``two reasonable ways'' are equivalent under
...
\begin{keyblock}
\textbf{Cell~17.} \emph{(Output) Implied Fed Funds Rate step plot.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L3_2_c17_implied_ff.png}\end{center}
...
expected-rate curve (blue) agree \emph{between} meetings and differ only
...
can be any real number, because each step is the \emph{expected} move,
...
exactly what the CME's \emph{FedWatch} tool does by inverting an options
chain.

\subsection{Comparing Across Quote Dates}\label{L3-sec:infer-compare}

\begin{keyblock}
\textbf{Cells~20--24.} \emph{Compare to the Historic Curve.}\\
...
\begin{keyblock}
\textbf{Cell~19.} \emph{(Output) Change in Expected Rates between quote dates.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L3_2_c19_change.png}\end{center}
...
\emph{downward} by roughly 30--70~bp at the back of the chain,
...
The same overlay, run across a \emph{short} interval (two weeks apart in
...
\subsection{Open Interest and Chain Liquidity}\label{L3-sec:infer-oi}

\begin{keyblock}
\textbf{Cells~25--26.} \emph{Looking at Open Interest on the Chain.}\\
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L3_2_c26_oi_scatter.png}\end{center}
...
\section{Discount Curve from SOFR Futures}\label{L3-sec:discount}
...
notebook (\texttt{3.3.~Discount via SOFR Futures.html}) has much more
...
\subsection{Setup --- Volume and Open Interest}\label{L3-sec:discount-oi}

\begin{keyblock}
\textbf{Cell~9.} \emph{(Output) Volume vs.\ Open Interest by Contract.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L3_3_c09_volume_oi.png}\end{center}
...
contract life-cycle --- is that M5 was sitting \emph{inside} its reference
...
lecturer drew: \emph{do not calibrate a discount curve from SOFR futures
...
\subsection{Reading a Futures Rate as a Forward Rate}\label{L3-sec:discount-forwardread}
...
\begin{enumerate}
  \item The contracts are zero-coupon in effect: the reference rate is a
        single-period money-market rate with no intermediate cashflows,
        so there is no bootstrap needed to extract a forward.
  \item The liquid front-end is densely populated, with a contract for
...
quite right --- the convexity adjustment from \S\ref{L3-sec:stir-tech}
applies --- but for short horizons it is a good first approximation.

\begin{keyblock}
\textbf{Cell~18.} \emph{(Output) Short-term rates from SOFR futures.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L3_3_c18_shortterm_rates.png}\end{center}
...
\subsection{Convexity Adjustment in Practice}\label{L3-sec:discount-conv}

\begin{keyblock}
\textbf{Cell~13.} \emph{(Output) Convexity bias and adjusted rates, Bloomberg vs.\ in-house.}
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/L3_3_c13_bloomberg_comp.png}\end{center}
\emph{Left panel:} size of the convexity bias ($r^{\text{fut}} -
...
10~years.\\
\emph{Right panel:} the convexity-adjusted forward rate under each
...
\begin{checkpoint}
\textbf{My note.} The pedagogical payoff of the convexity adjustment is
not the 2~bp bias correction --- it is the lesson that a futures contract
and a forward contract on the same underlying are \emph{not} the same
instrument, and the difference grows in the underlying's volatility and
the horizon squared. That intuition is what carries through to bond
futures and the cheapest-to-deliver story the lecturer previewed for
Lecture~4: whenever daily settlement is coupled to a rate that moves, the
long pays a small premium for the right to lock in the rate at the
